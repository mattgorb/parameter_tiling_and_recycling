...
parser.add_argument('--weight_path',
                    default='./pretrain/ssd300_mAP_77.43_v2.pth', type=str,
...
devkit_path_save = os.path.join(devkit_path, 'BinarySSD')
if not os.path.exists(devkit_path_save):
    os.makedirs(devkit_path_save)
...
def get_output_dir(phase):
    """Return the directory where experimental artifacts are placed.
    If the directory does not exist, it is created.
    A canonical path is built using the name from an imdb and a network
    (if not None).
    """
    filedir = os.path.join(args.save_folder, phase)
...
def get_voc_results_file_template(image_set, cls):
    ...
    filedir = os.path.join(devkit_path_save, 'results')
    if not os.path.exists(filedir):
        os.makedirs(filedir)
...
def write_voc_results_file(all_boxes, dataset):
    for cls_ind, cls in enumerate(labelmap):
        ...
        print('Writing {:s} VOC results file in {:s}'.format(cls, filename))
...
def do_python_eval(output_dir='output', use_07=True):
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    print("doing python evaluation in", output_dir)
...
def test_net(net, dataset):
    ...
    output_dir = get_output_dir(set_type)
