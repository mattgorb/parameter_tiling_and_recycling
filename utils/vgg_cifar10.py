...
def conv_init(cin, cout, layer_compression_factor):
...
class VGG_SMALL(nn.Module):
    ...
    def __init__(self, num_classes=10):
        ...
        self.conv0 = conv_init(3, 128, self.layer_compression_factors[0])
        self.bn0 = LearnedBatchNorm(128)
        self.conv1 = conv_init(128, 128, self.layer_compression_factors[1])
        self.pooling = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bn1 = LearnedBatchNorm(128)
        self.nonlinear = nn.ReLU(inplace=True)
        #self.nonlinear = nn.Hardtanh(inplace=True)
        self.conv2 = conv_init(128, 256, self.layer_compression_factors[2])
        self.bn2 = LearnedBatchNorm(256)
        self.conv3 = conv_init(256, 256, self.layer_compression_factors[3])
        self.bn3 = LearnedBatchNorm(256)
        self.conv4 = conv_init(256, 512, self.layer_compression_factors[4])
        self.bn4 = LearnedBatchNorm(512)
        self.conv5 = conv_init(512, 512, self.layer_compression_factors[5])
        self.bn5 = LearnedBatchNorm(512)

        self.fc = SubnetLinearTiledFull(512*4*4, num_classes, bias=False)
        self.fc.init(args, self.compression_factor, self.layer_compression_factors[6])
        ...
        #sys.exit()
        #conv.init(args, self.compression_factor,
